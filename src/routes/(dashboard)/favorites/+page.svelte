<script lang="ts">
	...
	// Query to fetch user's favorited prompts - only when user is signed in
	const favoritesQuery = $derived(
		clerkUserId ? useQuery(api.prompts.getFavoritedPrompts, { clerkUserId }) : null
	);
	const favorites = $derived(favoritesQuery?.data as Prompt[] | undefined);
	const isLoading = $derived(favoritesQuery?.isLoading ?? false);
...
<div class="space-y-6 p-6">
	...
	{:else if !favorites || favorites.length === 0}
		...
		>
			<div class="text-center">
				<h3 class="mb-2 text-lg font-semibold">No favorites yet</h3>
	...
	{:else}
		<!-- Favorites grid -->
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
			{#each favorites as prompt (prompt._id)}
				<div
